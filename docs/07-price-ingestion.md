...
>
> **운영 원칙 (2026-09-15):** 섹터의 흐름을 보는 데 집중한다. 단순한 규칙으로 일관되게 계산하고, 규칙 밖의 사건은 자동으로 보정하지 않고 특이사항으로 기록해 따로 본다 ([§11.2](#112-특이사항), [§14](#14-결정-기록)).
...
.venv\Scripts\python -m theme_radar prices index     --market KR               # 거래일 캘린더와 지수 일봉만 다시 받는다 (§12)
```
...
- 캐시는 개인 운영 저장소다. 갱신이 멈추면 Daum 단독으로 운영한다 ([§14](#14-결정-기록) D-6).
...
- 섹터 흐름에 주는 영향이 작으므로 근사를 유지하고 특이사항(G-3)으로 기록한다 (§14 D-7).
...
- 화면과 API에서 특이사항을 조회하는 방법은 후속 작업이다 ([§14](#14-결정-기록)).
...
**KR은 방법이 확인됐다.** [§15](#15-초기-적재-결과-2026-09-16)에 측정 결과를 적었다.
...
거래일 캘린더는 원래 시장 지수 시계열의 날짜로 만든다([§7](#7-한국-수집-절차), [§8](#8-미국-수집-절차)). 그때 받는 응답에 시가·고가·저가·종가가 이미 들어 있어, 날짜만 쓰고 버리는 대신 `market_index_daily`에 남긴다([02 §4.4](02-domain-and-data-model.md#44-market_index_daily)). 화면 오른쪽의 시장 지수 캔들([06 §3.6](06-ui-spec.md#36-시장-지수와-섹터-캔들))이 쓴다.

| 시장 | 지수 | 출처 | 요청 |
| --- | --- | --- | --- |
| KR | `KOSPI` | 네이버 `siseJson` (`NAVER_SISE`) | 거래일 캘린더와 같은 요청 하나 |
| US | `SPX` | yfinance `^GSPC` (`YFINANCE`) | 〃 |

- **새 출처를 붙이지 않는다.** 수집 요청 수가 늘지 않고, 캘린더와 지수가 같은 응답에서 나와 서로 어긋나지 않는다.
- 지수는 분할·배당 조정이 없어 받은 값을 그대로 둔다. 소급 갱신도 하지 않는다.
- 값이 어긋난 날(고가 < 종가처럼)은 **넣지 않는다.** 바로잡을 근거가 없는 한 날이라 버리고 그 자리는 차트에서 빈다. KR은 거래가 없던 날 시가·고가·저가가 0으로 오는데 그런 날도 같이 빠진다(§7.2).
- `backfill`·`daily`·`reconcile`은 캘린더를 갱신하므로 지수도 함께 최신이 된다. 지수만 다시 받으려면 `prices index --market KR`이다. 요청 하나라 몇 초면 끝난다.
...
- KR 액면병합이 239건으로 분할(56건)보다 많다. 07 §13 6항에서 본 2026년 병합 급증이 그대로 나타난다.
