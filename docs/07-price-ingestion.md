...
| G-3 | 복수 클래스 7개 라인의 가동 전 주식수 이력이 없다 | 가동일 값을 수정계수로 환산해 근사한다 | `DATA_GAP` 1건 |
...
│   ├── naver.py         # KR 원종가(모바일 일별시세), 수정 종가(siseJson) (직접 호출)
│   ├── daum.py          # KR 일별 상장주식수 (직접 호출)
│   ├── fdr_krx.py       # KR KRX 스냅샷 캐시, 상장폐지 목록 (FinanceDataReader)
│   ├── wiki.py          # US 구성종목 변경 이력 (직접 호출)
│   ├── yf.py            # US 종가, 분할 이벤트, 복수 클래스 주식수 (yfinance)
│   └── sec.py           # US 주식수 (직접 호출)
├── universe.py          # security, universe_membership 생성
├── collect_kr.py        # KR 백필·증분
├── collect_us.py        # US 백필·증분
├── restate.py           # 수정계수 소급 갱신 (9장)
├── validate.py          # 수집 단계 검증 (§11.1)
...
- 거래가 없던 날은 시가·고가·저가가 0이고 종가에 직전 값이 들어 있다. 현대홈쇼핑 2026-07-16 행이 `[0, 0, 0, 87300]`이다. 이런 날은 `trade_status = NO_TRADE`로 적재한다. 과거 구간에서는 거래정지와 무거래를 구분할 수 없지만, [03 §10](03-metrics-spec.md#10-예외-처리)은 둘 다 직전 종가를 캐리포워드하므로 계산 결과는 같다.
...
1. 매일 KR 수집 때 전 거래일 파일의 `Stocks`를 `shares_observation`(`source = FDR_KRX_CACHE`)에 적재한다.
...
- 편입 종목은 `valid_from = 효력일`, 편출 종목은 `valid_to = 효력일의 직전 거래일`로 둔다. 효력일 당일 개장 전에 반영된다는 해석이며, 구현 시 몇 건을 S&P 공지와 대조해 확인한다.
...
가동 전 구간(2025-01-01 ~ 가동 전일)은 가동일 값을 수정계수로 환산한 근사치를 쓴다. 식은 한국과 같다([§7.4](#74-상장주식수)). 근사 구간은 `DATA_GAP` 특이사항 1건으로 기록한다 (G-3).
...
| 시장 | 신호 | 방법 |
| --- | --- | --- |
| KR | 수정계수 변화 | 증분 수집 때 받은 `siseJson`으로 계수를 다시 계산해 저장값과 비교한다. `\|new / old − 1\| > 0.5 / close_adj + 1e-6`인 날이 하나라도 있으면 발생으로 본다 |
...
실측한 사례를 고정 테스트로 쓴다. 라이브러리 버전을 올릴 때도 이 테스트를 통과해야 한다. 모든 케이스에서 `restate` 전후 `market_cap`이 바뀌지 않아야 한다.
...
| KR | 207940 | 2025-10-30 | 인적분할 | 2025-10-27 `adj_factor` 1.471744, `action_type` `UNKNOWN`, `CORP_ACTION` 특이사항 1건 |
...
| C-2 | 원종가가 전일 대비 ±30%를 넘었는데 같은 날 기업행위 기록이 없다 | KR | 경고, 점검 큐 (신규상장·정리매매는 예외) |
| C-3 | 원종가가 전일 대비 ±50%를 넘었는데 분할 이벤트가 없다 | US | 경고, 점검 큐 |
| C-4 | 캘린더상 거래일인데 행이 없다 | 전체 | 경고. 상장 중인 종목이 3거래일 연속이면 차단 |
...
| C-8 | 표본 교차검증. KR은 WiseIndex 역산가, US는 Nasdaq 최근 종가와 대조한다 | 일 1회 50종목 | 불일치 2% 초과 시 경고 |
...
- 사건은 `daily`와 `backfill` 실행 끝에 만들고, 같은 사건은 한 번만 기록한다.
...
9. **종목 식별자.** KR은 영문자가 섞인 코드와 선행 0이 있으므로 문자열로 저장한다. pandas로 읽을 때도 `dtype={"Code": str}`를 준다. US는 표기 차이(`BRK.B`와 `BRK-B`)와 티커 변경·재사용이 있으므로 [02 §3.2](02-domain-and-data-model.md#32-security)의 `security_id`와 `security_alias`로 관리한다.
