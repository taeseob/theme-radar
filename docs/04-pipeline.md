...
- 구현 기술(Python, SQLite, FastAPI, ECharts)과 실행 방식은 [09-tech-stack.md](09-tech-stack.md)를 따른다.
...
이 문서의 "알림"(§5.1 포함)은 메일·메신저 발송이 아니다. 각 작업은 결과를 `batch_run`·`validation_result`([§7](#7-운영-메타데이터))에 남기고 실패하면 0이 아닌 종료 코드로 끝난다. 임계 초과와 실패는 `status` 명령으로 확인한다 ([09 §4.6](09-tech-stack.md#46-배치-실행과-운영)).
