...
- (추가, 2026-09-21) 오른쪽 패널의 두 캔들 차트는 `echarts.connect`로 묶어 세로선을 함께 세우고, 상자는 포인터가 들어온 차트에만 `tooltip.showContent`로 띄운다. 격자(`grid.left`)와 기간 축이 같아 세로선이 같은 자리에 선다. 툴팁 상자는 `confine`을 끄고 자리를 직접 계산해 포인터 왼쪽에 붙인다(06 §5.4). 상자 자리 계산은 순수 함수로 빼 헤드리스 크롬에서 따로 확인했다. ECharts가 상자를 새 자리로 미끄러뜨리는 동안 재면 값이 흔들려, 합성 이벤트로는 자리를 직접 재지 않는다.
- (추가, 2026-09-21) 이동평균 기준에 "이동평균 상승만" 거르개를 더하고(06 §3.5), 스냅샷에 "지수 대비"와 "상승률" 열을 넣었다(06 §4). 상승률은 이미 화면이 만들던 값이라 그대로 썼고(`web/js/ma.js`), 지수 대비는 오른쪽 패널이 받던 `/market/index` 응답을 `loadIndex`로 한 번만 받아 두 쪽이 나눠 쓰게 했다. 서버는 그대로다. 열이 열한 개가 되어 1280px 아래에서 고정 칸을 줄이고 1080px 아래에서 새 두 열을 감춘다. 폭 1600·1200·1000px 헤드리스 크롬 스크린샷으로 확인했다.
