...
| 내부 분석 용도다. 대시보드를 외부에 공개하지 않는다 | [07 §13](07-price-ingestion.md#13-그-밖의-고려사항) 3항 이용약관 |
...
| 상황 | 조치 |
| --- | --- |
| 대시보드를 다른 사람에게 공개하거나 여러 명이 쓰게 된다 | 이용약관 검토(07 §13 3항)를 먼저 한다. 그다음 PostgreSQL, 인증, 서버 배포를 함께 검토한다 |
...
- (추가, 2026-09-18) 섹터 시총 이동평균으로 보는 순위: 순위 기준 토글(기간 수익률 ↔ 이동평균 상승률, [06 §3.5](06-ui-spec.md#35-이동평균-기준)), "이동평균 위만" 필터, 스냅샷의 이격도 열·정렬, 시가총액 차트의 특이사항 표시. 값은 여전히 화면이 만들고(S-17), 서버는 섹터 전체의 기간 말 시총을 주는 `/sectors/market-caps`([05 §3.3](05-api-spec.md#33-get-sectorsmarket-caps)) 하나만 늘렸다. 26섹터 57주 46ms · S&P 500 11섹터 109주 43ms다. 이동평균 기간 수는 화면 하나로 모아 순위·이격도·시가총액 차트가 같은 `N`을 쓴다. 화면 계산(`web/js/ma.js`)은 임시 점검 페이지를 헤드리스 크롬으로 돌려 확인했다.
- (추가, 2026-09-19) 범프 차트 더블클릭 강조와 오른쪽 시장 배경: 섹터 더블클릭으로 강조를 넣고 빼며(06 §3.2), 차트 오른쪽에 시장 지수 캔들과 고른 섹터의 시가총액 캔들을 둔다(06 §3.6). 지수는 거래일 캘린더를 만들며 이미 받던 일봉을 `market_index_daily`(마이그레이션 0005)에 남겨 쓴다(02 §9 S-18). 기존 DB는 `init-db` 뒤 `prices index --market KR|US` 한 번이면 채워진다(KR 0.3초·US 7.4초). 시가총액 차트 모듈은 지수와 섹터를 함께 그리도록 `web/js/candle-chart.js`로 옮기고 한 화면에 여러 개를 둘 수 있게 했다. 더블클릭·클릭이 겹치는 부분은 헤드리스 크롬에서 실제 이벤트를 보내 확인했다.
- (추가, 2026-09-21) 순위 차트와 지수 패널의 너비는 가운데 손잡이로 나눈다(06 §3.6). 차트 높이와 같이 브라우저에 저장한다. 폭이 바뀌면 이미 있던 ResizeObserver가 두 쪽을 다시 그린다. 범프 차트 x축 라벨은 폭에 따라 자리가 모자랄 수 있어 ECharts가 간격을 고르게 바꿨다.
- 2단계 적재 결과와 그 과정에서 고친 규칙은 [07 §15](07-price-ingestion.md#15-초기-적재-결과-2026-09-16), [§3.5](07-price-ingestion.md#35-구현-착수-시-재확인-2026-09-16)에 기록했다.
