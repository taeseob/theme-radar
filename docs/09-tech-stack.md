...
- 파생 결과는 `(universe, scheme, period_type)` 조합 하나를 한 트랜잭션으로 쓴다.
...
[06 §3](06-ui-spec.md#3-범프-차트-메인) 범프 차트 요구와 ECharts 기능의 대응이다. **미검증:** 문서 지식 기준이며, [§10](#10-구현-순서) 5단계 착수 전에 프로토타입으로 확인한다.

| 06 요구 | ECharts 기능 |
| --- | --- |
| 1위가 최상단 | `yAxis.inverse: true` |
| 선 hover 시 강조, 나머지 감쇠 | `emphasis.focus: 'series'`와 `blur` 상태 스타일 |
| 오른쪽 끝 그룹명 라벨, 겹침 회피 | 라인 시리즈 `endLabel` + `labelLayout.moveOverlap: 'shiftY'` |
...
| 수익률 부호별 점 테두리 | 데이터 항목별 `itemStyle.borderType` (`solid` / `dashed`) |
| 잠정 기간 점선 | 구간별 선 스타일은 내장 기능이 없다. 마지막 구간만 담은 보조 시리즈를 점선으로 겹쳐 그린다 |
...
.venv\Scripts\python -m theme_radar status                     # 최근 batch_run과 실패·경고 검증 결과
.venv\Scripts\python -m theme_radar backup                     # DB 백업
...
- 모든 작업은 `batch_run`, `validation_result`([04 §7](04-pipeline.md#7-운영-메타데이터))에 결과를 남기고, 실패하면 0이 아닌 종료 코드로 끝난다. 작업 스케줄러 기록에서 실패가 보인다.
- `status` 명령이 최근 실행, 실패, 경고(V-8, V-9, 모니터링 임계 초과)를 한 화면에 보여 준다.
...
- 2단계 적재 결과와 그 과정에서 고친 규칙은 [07 §14](07-price-ingestion.md#14-초기-적재-결과-2026-09-16), [§3.5](07-price-ingestion.md#35-구현-착수-시-재확인-2026-09-16)에 기록했다. C-8(표본 교차검증)은 6단계로 미뤘다.
